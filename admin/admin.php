...
if (isset($_GET['edit'])) {
    $id = $_GET['edit'];
    $stmt = $pdo->prepare("SELECT * FROM bacaan_sholat WHERE id = :id");
    $stmt->execute([':id' => $id]);
...
<head>
  <meta charset="UTF-8">
  <title>Admin - CRUD Bacaan Sholat</title>
  <style>
    body {
...
<body>
  <header>
    <h1>Halaman Admin - Bacaan Sholat Fardhu</h1>
  </header>
  <main>
    ...
    <table>
      ...
      <tbody>
        <?php
        $stmt = $pdo->query("SELECT * FROM bacaan_sholat ORDER BY jenis_sholat, urutan");
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
          ...
          echo "</td>
            <td>
              ...
              <a class='action-btn' href='hapus.php?id={$row['ID']}' onclick=\"return confirm('Yakin hapus?')\">Hapus</a>
            </td>
          </tr>";
        }
        ?>
